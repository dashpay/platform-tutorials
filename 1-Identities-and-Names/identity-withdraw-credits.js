...
const withdrawCredits = async () => {
  ...
  console.log(
    'Identity balance before transfer: ',
    identity.balance,
  );
  ...
  const metadata = await client.platform.identities.withdrawCredits(identity, withdrawalAmount, {
    toAddress,
  });
...
withdrawCredits()
  .then((d) =>
    console.log('Identity balance after withdrawal: ', d.balance),
  )
