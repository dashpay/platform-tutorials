...
function App() {
  ...
  const { status, sdk, identityId, contractId, contractOwnerId, log, browseOnly } = session
  ...
  return (
    <>
      <Toaster position="bottom-center" theme="dark" richColors duration={2200} />
      ...
      >
        ...
        {tab === 'mint' && (
          <section className="relative">
            ...
            {status === 'authenticated' && contractOwnerId && identityId !== contractOwnerId && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-lg bg-bg/55 backdrop-blur-sm">
                <p className="max-w-sm text-center text-sm text-ink-2">
                  Only the contract owner can mint new cards. Register your own
                  new contract in Settings to try this feature.
                </p>
                <button
                  type="button"
                  onClick={() => setLoginOpen(true)}
