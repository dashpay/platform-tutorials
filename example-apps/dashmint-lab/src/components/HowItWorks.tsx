...
const OPERATIONS: { op: string; file: string; method: string }[] = [
  { op: 'Connect testnet',    file: 'src/dash/client.ts',        method: 'EvoSDK.testnetTrusted()' },
  { op: 'Derive keys',        file: 'src/dash/keyManager.ts',    method: 'wallet.deriveKeyFromSeedWithPath' },
  { op: 'Deploy contract',    file: 'src/dash/contract.ts',      method: 'sdk.contracts.publish' },
  { op: 'Mint card',          file: 'src/dash/mintCard.ts',      method: 'sdk.documents.create' },
  { op: 'Transfer card',      file: 'src/dash/transferCard.ts',  method: 'sdk.documents.transfer' },
  { op: 'Set / remove price', file: 'src/dash/setPrice.ts',      method: 'sdk.documents.setPrice' },
  { op: 'Purchase card',      file: 'src/dash/purchaseCard.ts',  method: 'sdk.documents.purchase' },
  { op: 'Burn (delete)',      file: 'src/dash/burnCard.ts',      method: 'sdk.documents.delete' },
  { op: 'Query cards',        file: 'src/dash/queries.ts',       method: 'sdk.documents.query' },
]
...
const COMPARISONS: { dimension: string; dash: string; ethereum: string }[] = [
  ...
  },
  {
    ...
    ethereum: 'Upload metadata to IPFS, then call a contract function to mint a token',
  },
  {
    dimension: 'Transfer',
  ...
  },
  {
    ...
    ethereum: 'Requires separate marketplace contracts (OpenSea, Seaport, etc.)',
  },
  {
  ...
  },
  {
    ...
    ethereum: 'Learn Solidity + Foundry/Hardhat toolchain + ABI encoding + frontend integration',
  },
  {
    dimension: 'Querying',
    dash: 'Built-in document queries with where-clause filters',
    ethereum: 'Contract view functions, event logs, or external indexers (The Graph)',
  },
]

const READING_ORDER = [
  { file: 'src/dash/mintCard.ts', desc: 'Simplest create flow — one SDK call.' },
  { file: 'src/dash/withAuthedCard.ts', desc: 'Shared prelude for transfer, setPrice, purchase, burn.' },
  { file: 'src/session/SessionContext.tsx', desc: 'SDK instance, identity, contract ID, activity log.' },
  { file: 'src/components/', desc: 'UI. Modals call src/dash/ functions directly.' },
]
    ...
    <div className="mx-auto flex max-w-[860px] flex-col gap-3.5">
      ...
      <section className="rounded-xl border border-line bg-surface p-5">
        ...
        <p className="mt-2 text-[13.5px] leading-[1.55] text-ink-2">
          ...
          Platform. Each card is a document owned by an identity —
          transferable, tradeable, and stored on-chain.
        </p>
        ...
        <div className="mt-4 rounded-lg border border-line bg-bg p-3.5">
          ...
          <p className="text-[13px] leading-[1.55] text-ink-2">
            These cards aren't tokens — they're documents
            stored on Dash Platform:
          </p>
