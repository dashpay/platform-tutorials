...
- **[test/](test/)** — Vitest + Testing Library. Co-located by subject (e.g. `CardTile.test.tsx`, `SessionContext.test.tsx`). Default env is `node`; tests that need DOM opt in with `// @vitest-environment jsdom`.
