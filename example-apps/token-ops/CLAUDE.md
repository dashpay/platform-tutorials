...
- **[src/session/](src/session/)** — `SessionContext.tsx` provides `{ status, error, sdk, keyManager, identityId, contractId, setContractId, log, login, enterReadOnly, logout }`. `login(mnemonic, identityIndex=0)` builds an `IdentityKeyManager`; `enterReadOnly()` connects the SDK with no keys (`App.tsx` auto-enters it on `status === "idle"`). Network is hardcoded to **testnet**. The mnemonic is never stored in state or localStorage — only `contractId` is persisted. Consumer hook: `useSession.ts`.
...
- Writes the three member IDs to **`token-ops/.env`** as `VITE_TOKEN_OPS_MEMBER_1_ID` / `_2_ID` / `_3_ID` (owner index 0 gets no env key). The Account tab defaults its register-contract group members to these.
