...
- **WIF sign-in** ([loginWithPrivateKey.ts](src/dash/loginWithPrivateKey.ts)): `PrivateKey.fromWIF` → `privateKey.getPublicKeyHash()` → `sdk.identities.byPublicKeyHash(hash)` first, then paginated `byNonUniquePublicKeyHash(hash, startAfter)` if the unique lookup misses. Matching accepts full ECDSA and ECDSA HASH160 keys only, gates on AUTHENTICATION + HIGH/CRITICAL + enabled, and requires an exact identity ID when multiple identities match. Explicit IDs are fetched directly and verified; result order is never used for selection.
