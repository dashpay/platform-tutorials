...
- **[src/dash/](src/dash/)** — one file per Platform SDK operation. Each primarily exports an async function with a leading JSDoc block naming the SDK method it wraps. No hooks, no UI wrappers — the SDK call is the function. Some modules also export tightly-related helpers alongside the wrapper (e.g. [contract.ts](src/dash/contract.ts) exports `ANCHOR_CONTRACT`, `loadStoredContractId`/`saveContractId`/`clearStoredContractId`, and `refreshContractCache` next to `registerContract`); keep these co-located with their SDK call rather than splitting them out.
...
- **[src/session/](src/session/)** — `SessionContext.tsx` provides the context (`status`, `error`, `sdk`, `keyManager`, `identityId`, `contractId`, `setContractId`, `log`, `login`, `enterReadOnly`, `logout`) and `useSession.ts` is the consumer hook. Session statuses: `idle | connecting | readonly | authenticated | error`. Mnemonic lives only in the keyManager closure — never in state, never in localStorage. [App.tsx](src/App.tsx) auto-calls `enterReadOnly()` on `status === "idle"`, so the app boots into a connected read-only state without a login prompt.
...
- **[test/](test/)** — Vitest + Testing Library. All test files live in this flat directory per the `include` pattern in [vite.config.ts](vite.config.ts) (`test/**/*.test.{ts,tsx}`) and are named after the subject under test (e.g. `AnchorForm.test.tsx`, `SessionContext.test.tsx`, `dash.test.ts`) — they are **not** colocated next to source files, and the directory is **not** mirrored against `src/`. Default Vitest env is `node`; component tests opt into DOM with a `// @vitest-environment jsdom` pragma at the top of the file.
...
- Read-only mode (`session.status === "readonly"`) sets `keyManager` to `null`. Any write path (`createAnchor`, `registerContract`) must guard for an authenticated session.
