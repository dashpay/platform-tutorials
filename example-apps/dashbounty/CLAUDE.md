...
- **[src/dash/](src/dash/)** — one file per Platform SDK operation, each with a leading JSDoc block naming the SDK method(s) it wraps:
  ...
  - `types.ts` — hand-typed `DashSdk`/`DashKeyManager` subset, extended with `tokens.freeze/unfreeze/destroyFrozen/identityTokenInfos`, `group.info/members/actions/actionSigners`
...
- **The panel roster is fixed at contract registration.** Platform's contract-update validation rejects any change to an existing group with `DataContractUpdateActionNotAllowedError` ("change group at position 0 is not allowed") — there is no live add/remove/swap-member operation, owner-signed or otherwise. New groups can be appended at higher positions, but this token gates freeze/unfreeze/destroy on group 0 and locks `mainControlGroupCanBeModified` to `NoOne`, so an appended group can never take over token governance. The only way to run a different panel is to register (or select) a new contract with the desired members. `RosterView` is therefore read-only.
...
- **Existing groups cannot be changed after registration — at all.** Membership lives in `dataContract.groups[n].members`, but a `sdk.contracts.update(...)` that touches an existing group is rejected on-chain with `DataContractUpdateActionNotAllowedError` ("change group at position N is not allowed"). Don't add a roster-mutation flow; changing the panel requires a new contract.
