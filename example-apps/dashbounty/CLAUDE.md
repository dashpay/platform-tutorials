...
- **[src/dash/](src/dash/)** — one file per Platform SDK operation, each with a leading JSDoc block naming the SDK method(s) it wraps:
  ...
  - `types.ts` — hand-typed `DashSdk`/`DashKeyManager` subset, extended with `tokens.freeze/unfreeze/destroyFrozen/identityTokenInfos`, `group.info/members/actions/actionSigners`, `contracts.update`
  ...
  - `panel.ts` — `fetchPanelInfo`, `fetchPanelMembers`, `isPanelMember`
  - `updatePanelRoster.ts` — owner-authorized contract update that mutates `dataContract.groups[0].members`
...
- **`freezeRules` / `unfreezeRules` / `destroyFrozenFundsRules`** are all `ChangeControlRules({ authorizedToMakeChange: AuthorizedActionTakers.Group(0), adminActionTakers: AuthorizedActionTakers.Group(0) })` — only the panel, acting together, can freeze/unfreeze/destroy.
...
- **Panel roster changes are owner-key-authorized, not group-gated.** `DataContractConfig` has no `ChangeControlRules`-style admin gate of its own — `sdk.contracts.update(...)` is unconditionally signed by the contract owner. There is no way to make roster rotation itself require 2-of-3 panel approval. `updatePanelRoster.ts` fetches the contract, mutates `dataContract.groups[0].members` (a `Map`), bumps `.version`, and calls `sdk.contracts.update(...)` — directly modeled on `2-Contracts-and-Documents/contract-update-minimal.mjs`, substituting a `.groups` mutation for that tutorial's `.schemas` mutation.
...
- **Rotate roster**: fetch → mutate `.groups[0].members` + bump `.version` → `sdk.contracts.update({ dataContract, identityKey, signer })`
...
- **There is no "rotate group members" facade call.** Membership lives only in `dataContract.groups[n].members`; the only path to change it is a full `sdk.contracts.update(...)`.
...
- Roster-rotation contract updates bump `.version` exactly like schema updates — `updatePanelRoster` fetches fresh immediately before mutating to avoid a stale-version conflict if two roster edits race.
